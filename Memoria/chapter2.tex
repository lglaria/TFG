...
\end{equation}
Para $n > 20$ la probabilidad anterior prácticamente cero (Para función SHA-256 se tiene que $n = 256$).
 Para garantizar una probabilidad de al menos el 0.5 se tiene que:
\begin{equation}
k=\frac{1}{2}2^{n} = 2^{n-1} \approx 2^{n}
\end{equation}
Siendo k es el número de cadenas que hay que estudiar. En este caso el coste es prácticamente el mismo que el de comprobar todas las posibles cadenas que produce nuestra función hash.
